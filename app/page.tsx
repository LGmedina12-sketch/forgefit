...
const activityBoost: Record<MobilityActivity, Area[]> = {
...
export default function Page() {
  ...
  function generateWorkout() {
    ...
    const splitMatched = exercises.filter((ex) => matchesSplit(ex, split, focus));
    const splitSource = splitMatched.length >= Math.min(3, count) ? splitMatched : exercises;
    const scored = splitSource.map((ex) => {
      ...
      if (!isStrict && split !== 'full' && split !== 'mma') score -= 45;
    ...
    const r = workoutRerolls + 1; setGenerated(workout); setWorkoutRerolls(r); setRecentExerciseIds((old) => [...workout.map((x) => x.id), ...old].slice(0, 14)); setWorkoutMessage(workout.length ? `Workout #${r}. Split gate: ${splitAllowed.join(', ')}.${offSplit ? ` ${offSplit} fallback move used because matching options were limited.` : ''}${tiredFocus.length ? ` ${tiredFocus.join(', ')} still needs rest, so volume shifted when possible.` : ''}` : 'No exercises matched your split and equipment.');
  }
...
function getAllowedMusclesForSplit(split: SplitKey, focus: string[]) { const base: Record<SplitKey, string[]> = { push: ['chest', 'shoulders', 'arms', 'triceps', 'core'], pull: ['back', 'shoulders', 'arms', 'biceps', 'forearms', 'core'], legs: ['legs', 'glutes', 'hamstrings', 'quads', 'core'], lower: ['legs', 'glutes', 'hamstrings', 'quads', 'core'], upper: ['back', 'chest', 'shoulders', 'arms', 'biceps', 'triceps', 'forearms', 'core'], full: muscles, mma: muscles }; return Array.from(new Set([...base[split], ...expandMuscles(focus)])); }
function matchesSplit(ex: Exercise, split: SplitKey, focus: string[]) { const groups = expandMuscles(ex.muscle_groups); const allowed = getAllowedMusclesForSplit(split, focus); if (split === 'full' || split === 'mma') return groups.some((m) => allowed.includes(m)); const hardGroups = groups.filter((m) => m !== 'core'); return hardGroups.length ? hardGroups.some((m) => allowed.includes(m)) && !hardGroups.some((m) => !allowed.includes(m) && !['forearms', 'biceps', 'triceps'].includes(m)) : allowed.includes('core'); }
