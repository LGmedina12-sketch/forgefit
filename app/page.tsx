...
const equipment = ['bodyweight', 'dumbbell', 'machine', 'cable', 'smith machine', 'resistance band', 'bench', 'bar', 'box', 'medicine ball', 'barbell', 'kettlebell'];
...
export default function Page() {
  ...
  const [workoutEquipment, setWorkoutEquipment] = useState<string[]>(['bodyweight', 'dumbbell', 'machine', 'cable', 'smith machine', 'bench']);
  ...
  useEffect(() => {
    async function load() {
      ...
      const [{ data: ex, error: exError }, { data: mob, error: mobError }] = await Promise.all([
        supabase.from('exercises').select('*').order('name'),
        supabase.from('mobility_drills').select('*').order('target_area'),
      ]);
      setExercises((ex as Exercise[]) ?? []);
      setDrills((mob as Drill[]) ?? []);
      if (exError) setWorkoutMessage(`Exercise load error: ${exError.message}`);
      if (mobError) setRoutineMessage(`Mobility load error: ${mobError.message}`);
  ...
  function pickDay(day: SplitDay, i: number) { setDayIndex(i); setSplit(day.split); setFocus(day.muscles); setRoutineKey(day.activity === 'legs' ? 'legs_pre' : day.activity === 'push' || day.activity === 'pull' ? 'upper_pre' : day.activity === 'mma' ? 'mma_pre' : 'daily_reset'); setWorkoutMessage(`${day.label} selected. Split gate, muscle focus, and mobility routine updated.`); }
  ...
  function generateWorkout() {
    ...
    const splitAllowed = getAllowedMusclesForSplit(split, focus);
    const equipmentMatched = exercises.filter((ex) => matchesEquipment(ex.equipment_needed, workoutEquipment));
    const splitMatched = equipmentMatched.filter((ex) => matchesSplit(ex, split, focus));
    const splitSource = splitMatched.length >= Math.min(3, count) ? splitMatched : equipmentMatched;
    const scored = splitSource.map((ex) => {
      const readiness = getExerciseReadiness(ex, recoveryMap);
      const groups = expandMuscles(ex.muscle_groups);
      const isStrict = matchesSplit(ex, split, focus);
      let score = isStrict ? 35 : 5;
      score += Math.random() * 10;
      score += groups.filter((m) => splitAllowed.includes(m)).length * 9;
      score += groups.filter((m) => expandMuscles(focus).includes(m)).length * 6;
      if (goalText.includes('mma') && ['power', 'conditioning', 'core'].includes(ex.category)) score += 5;
      if (goalText.includes('strength') && ex.category === 'strength') score += 6;
      if (goalText.includes('muscle') && ex.category === 'hypertrophy') score += 6;
      if (goalText.includes('conditioning') && ex.category === 'conditioning') score += 5;
      if (recentExerciseIds.includes(ex.id)) score -= 8;
    ...
    const offSplit = workout.filter((ex) => !matchesSplit(ex, split, focus)).length;
    const r = workoutRerolls + 1; setGenerated(workout); setWorkoutRerolls(r); setRecentExerciseIds((old) => [...workout.map((x) => x.id), ...old].slice(0, 14)); setWorkoutMessage(workout.length ? `Workout #${r}. ${splitMatched.length} matching exercises available.${offSplit ? ` ${offSplit} fallback move used because matching options were limited.` : ''}${tiredFocus.length ? ` ${tiredFocus.join(', ')} still needs rest, so volume shifted when possible.` : ''}` : `No exercises matched. Loaded ${exercises.length} exercises, ${equipmentMatched.length} match your equipment. Try adding equipment.`);
  }
...
function matchesEquipment(required: string[] = [], selected: string[]) { const req = required.map((x) => x.toLowerCase()); if (!req.length || req.includes('bodyweight')) return true; return req.some((item) => selected.includes(item)); }
function expandMuscles(list: string[]) { const set = new Set<string>(); list.forEach((raw) => { const m = raw.toLowerCase(); set.add(m); if (['legs', 'quads', 'hamstrings', 'glutes', 'calves'].includes(m)) set.add('legs'); if (m === 'legs') ['quads', 'hamstrings', 'glutes', 'calves'].forEach((x) => set.add(x)); if (['arms', 'biceps', 'triceps', 'forearms'].includes(m)) set.add('arms'); if (m === 'arms') ['biceps', 'triceps', 'forearms'].forEach((x) => set.add(x)); if (['lats', 'upper back', 'rear delts'].includes(m)) set.add('back'); if (m === 'rear delts') set.add('shoulders'); if (m === 'full body') ['legs', 'back', 'chest', 'shoulders', 'arms', 'core'].forEach((x) => set.add(x)); if (m === 'hip flexors') { set.add('core'); set.add('legs'); } }); return Array.from(set); }
function getAllowedMusclesForSplit(split: SplitKey, focus: string[]) { const base: Record<SplitKey, string[]> = { push: ['chest', 'shoulders', 'arms', 'triceps', 'core'], pull: ['back', 'shoulders', 'arms', 'biceps', 'forearms', 'core'], legs: ['legs', 'glutes', 'hamstrings', 'quads', 'calves', 'core'], lower: ['legs', 'glutes', 'hamstrings', 'quads', 'calves', 'core'], upper: ['back', 'chest', 'shoulders', 'arms', 'biceps', 'triceps', 'forearms', 'core'], full: muscles, mma: muscles }; return Array.from(new Set([...base[split], ...expandMuscles(focus)])); }
function matchesSplit(ex: Exercise, split: SplitKey, focus: string[]) { const groups = expandMuscles(ex.muscle_groups); const allowed = getAllowedMusclesForSplit(split, focus); if (split === 'full' || split === 'mma') return groups.some((m) => allowed.includes(m)); return groups.some((m) => allowed.includes(m) && m !== 'core') || (groups.length === 1 && groups[0] === 'core'); }
